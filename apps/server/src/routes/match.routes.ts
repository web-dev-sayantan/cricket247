...
matchRoutes.get("/", async (c) => {
  try {
    ...
    const limit = Math.min(
      Number.parseInt(
        c.req.query("limit") || String(PAGINATION.DEFAULT_LIMIT),
        10
      ),
...
matchRoutes.get("/:id/scorecard", async (c) => {
  const parsedId = idRouteParamSchema.safeParse(c.req.param("id"));
  if (!parsedId.success) {
    return errorResponse(c, "Invalid match ID");
  }

  const inningsNumberQuery = c.req.query("inningsNumber");
  const includeBallByBallQuery = c.req.query("includeBallByBall");

  const inningsNumber = inningsNumberQuery
    ? Number.parseInt(inningsNumberQuery, 10)
    : undefined;
  const includeBallByBall = includeBallByBallQuery === "true";

  if (inningsNumberQuery && Number.isNaN(inningsNumber ?? Number.NaN)) {
    return errorResponse(c, "Invalid innings number");
  }

  try {
    const scorecard = await getMatchScorecard(parsedId.data, {
      inningsNumber,
      includeBallByBall,
    });
    ...
    return successResponse(c, scorecard);
  } catch (_error) {
    return errorResponse(c, "Failed to fetch scorecard", 500);
  }
});
...
matchRoutes.post("/", requireAuth, requireAdmin, async (c) => {
  ...
  if (!parsedBody.success) {
    return errorResponse(
      c,
      parsedBody.error.issues[0]?.message ?? "Invalid match payload"
    );
  }
