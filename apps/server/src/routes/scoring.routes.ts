...
scoringRoutes.post('/ball', requireScorer, async (c) => {
  ...
  if (!parsedBody.success) {
    return errorResponse(c, parsedBody.error.issues[0]?.message ?? 'Invalid ball payload');
  }
...
scoringRoutes.post('/innings/start', requireScorer, async (c) => {
  ...
  if (!parsedBody.success) {
    return errorResponse(c, parsedBody.error.issues[0]?.message ?? 'Invalid innings payload');
  }
