...
CREATE TABLE `innings` (
	...
	CONSTRAINT `fk_innings_opening_striker_id_players_id_fk` FOREIGN KEY (`opening_striker_id`) REFERENCES `players`(`id`),
	CONSTRAINT `fk_innings_opening_non_striker_id_players_id_fk` FOREIGN KEY (`opening_non_striker_id`) REFERENCES `players`(`id`),
	CONSTRAINT `fk_innings_opening_bowler_id_players_id_fk` FOREIGN KEY (`opening_bowler_id`) REFERENCES `players`(`id`)
);
--> statement-breakpoint
CREATE TABLE `match_formats` (
	...
	`is_follow_on_allowed` integer DEFAULT false NOT NULL,
...
CREATE TABLE `matches` (
	...
	`follow_on_allowed_snapshot` integer DEFAULT false NOT NULL,
	`players_per_side` integer DEFAULT 11 NOT NULL,
...
CREATE TABLE `players` (
	...
	`created_at` integer DEFAULT (unixepoch() * 1000) NOT NULL,
	`updated_at` integer DEFAULT (unixepoch() * 1000) NOT NULL,
	CONSTRAINT `fk_players_user_id_user_id_fk` FOREIGN KEY (`user_id`) REFERENCES `user`(`id`) ON DELETE SET NULL
);
--> statement-breakpoint
...
CREATE TABLE `tournaments` (
	...
	`player_of_the_tournament_id` integer,
	...
	CONSTRAINT `fk_tournaments_player_of_the_tournament_id_players_id_fk` FOREIGN KEY (`player_of_the_tournament_id`) REFERENCES `players`(`id`)
);
--> statement-breakpoint
CREATE TABLE `user` (
	`id` integer PRIMARY KEY,
