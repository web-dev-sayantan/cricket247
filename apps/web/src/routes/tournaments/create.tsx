...
function getStructureStepError(values: TournamentWizardValues) {
  if (values.structure.template === "grouped_league_with_playoffs") {
    if (values.structure.groupCount < 2) {
      return "Group count must be at least 2";
    }
    if (values.structure.advancingPerGroup < 1) {
      return "Advancing per group must be at least 1";
    }
...
function RouteComponent() {
  ...
  const [values, setValues] = useState(() =>
    getDefaultWizardValues(new Date())
  );
  ...
  }, [teams, values.teams.existingTeamIds]);
  ...
  const createTournamentMutation = useMutation({
    ...
    onError: (error) => {
      setGlobalError(error.message || "Failed to create tournament");
        ...
        <div className="mx-auto w-full max-w-2xl space-y-4 px-4 py-6 md:px-6 md:py-8">
          <header className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm md:text-base">
              You need admin access to create tournaments.
            </p>
          </header>
  ...
  const validateCurrentStep = () => {
    setGlobalError(null);
    const error = getCurrentStepError(currentStep, values);
    if (error) {
        ...
        <div className="space-y-3 rounded-lg border p-4">
          ...
          <FieldGroup className="md:grid md:grid-cols-4">
            ...
            <Field className="justify-end">
              ...
              <Button onClick={addInlineTeam} type="button" variant="outline">
                <Plus />
                Add Team
              </Button>
            </Field>
              ...
              {selectedExistingTeams.map((team, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      disabled={index === selectedExistingTeams.length - 1}
              ...
              {values.teams.createTeams.map((team, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      disabled={index === values.teams.createTeams.length - 1}
    ...
    <div className="min-h-screen bg-background pb-10">
      <div className="mx-auto w-full max-w-4xl space-y-5 px-4 py-6 md:px-6 md:py-8">
        <header className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm md:text-base">
            Build a tournament from scratch with teams and template-based
            structure.
          </p>
        </header>
        ...
        <Card className="rounded-xl">
          ...
          <CardContent className="space-y-5 pt-5">
            ...
            {!isLookupLoading && currentStep === 0 ? renderBasicsStep() : null}
            {!isLookupLoading && currentStep === 1 ? renderTeamsStep() : null}
            {!isLookupLoading && currentStep === 2
              ? renderStructureStep()
              : null}
            {!isLookupLoading && currentStep === 3 ? renderReviewStep() : null}
            ...
            <FieldDescription>
              ...
              fixtures after tournament creation.
            </FieldDescription>
